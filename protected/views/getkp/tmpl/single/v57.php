<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <style>
        p{mso-line-height-rule:exactly !important; line-height: 13pt;}
    </style>
</head>
<body style="background: #FFFFFF; margin: 0; padding: 0; border: 0;">
    <style>
        p{mso-line-height-rule:exactly !important; line-height: 13pt;}
    </style>
    <table cellspacing="0" cellpadding="0" width="800" align="center" bgcolor="#808080"  style="width: 840px; background: #808080; margin: 0 auto; padding: 0px; border: 0;">
        <tr>
            <td style="padding: 20px 0 20px;">
                <!-- START -->
                
                <table width="800" cellspacing="0" cellpadding="0" align="center" bgcolor="#f39314"  style="border:0; border-collapse: collapse; border-bottom: 2px solid white;">
                    <tr>
                        <td style="width: 170px; padding: 10px 20px 10px 10px; ">
                            <a href="http://www.lbr.ru/" title="Сайт ЛБР-агромаркет" style="border: 0; text-decoration: none;"> 
                                <img src="cid:1" width="160" height="64" style="border: 0; float: left;" alt="Нажмите отобразить рисунки">
                            </a>
                        </td>
                        <td style="text-align: right; padding-bottom: 10px; padding-right: 10px" valign="bottom">
                        
                        <table width="600" cellspacing="0" cellpadding="0" align="center" bgcolor="#f39314"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td style="padding-top: 10px;">
                                        <p style="padding: 0; font-size: 12pt; font-style: italic; font-family: Calibri; color: white; margin: 0; text-align: left; line-height: 13pt;"><?php echo $data['client']; ?>,</p>
                                        <p style="padding: 0; font-size: 12pt; font-style: italic; font-family: Calibri; color: white; margin: 0; text-align: left; line-height: 13pt;">Вы являетесь для нас очень важным партнером. В этом месяце мы подобрали для Вас самые актуальные предложения по технике.</p>
                                        <p style="padding: 0; font-size: 12pt; font-style: italic; font-family: Calibri; color: white; margin: 0; text-align: left; line-height: 13pt;">Теперь выбор нужной машины или агрегата стал еще проще – на расстоянии одного клика и нескольких минут</p>
                                    </td>
                                </tr>
                        </table>
                        
                        </td>
                    </tr>
                </table>
                <!-- Разделитель -->
                <table width="800" height="20" cellspacing="0" cellpadding="0" align="center" bgcolor="#808080"  style="height: 20px; border:0; border-collapse: collapse; overflow: hidden;  border-top: 3px solid #f39314;">
                    <tr style="line-height: 0; height: 20px;"><td style="line-height: 0; height: 20px;"><span style="overflow: hidden; margin: 0; padding: 0; color: #808080; line-height: 0; height: 20px;">_</span></td></tr>
                </table>
                <!-- Первая строка с большим блоком -->
                <table width="800" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/top_line_white.jpg" width="800" height="8" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 20px 0px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="top" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td colspan="2">
                                        <span style="font-size: 13pt; font-weight: bold; font-family: Calibri; color: #E31E24; margin: 0;  padding-right: 10px; text-transform: uppercase;">Новинка</span>
                                        <span style="font-size: 13pt; font-weight: bold; font-family: Calibri; color: black; text-transform: uppercase;"> Пневматическая пропашная селка точного высева GreatPlains – YP-825А</span>
                                    </td>
                                </tr>
                                <tr valign="top">
                                    <td>
                                        <table width="310" cellspacing="0" cellpadding="0" align="top" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                            <tr>
                                                <td style="padding: 20px 10px 0 0;">
                                                    <p style="font-size: 10pt; font-family: Calibri; color: black; margin: 0; line-height: 19px; text-align: justify;">На Западе есть поговорка, что «хорошие подарки дарят в небольших упаковках».  И это c полным основанием можно отнести к новой модели от GreatPlains.  Пришло время воспользоваться технологиями, которые были раньше доступны  только в  технике для крупных хозяйств. Инженеры  GreatPlains  разработали эти компактные сеялки с системой подачи семян AIR-PRO, подняв на новый уровень точность высева: до 99.9%</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 20px 10px 0 0; text-align: right;">
                                                    <a href="http://www.lbr.ru/tehnika/vozdelyvanie-propashnykh-kultur/seyalki-tochnogo-vyseva/seyalki-yp-825a/?utm_source=http%3A%2F%2Fwww.lbr.ru%2Fgetkp%2Ftest%2Fv%2Fv57%2F&utm_medium=email&utm_content=GREAT%20PLAINS%20YP-825A&utm_campaign=GREAT%20PLAINS%20YP-825A" title="Пневматические пропашные сеялки точного высева YP-825A" style="border: 0; text-decoration: none;">
                                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/more.jpg" width="113" height="25" style="border:0;">
                                                    </a>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                    <td>
                                        <img src="cid:2" width="450" height="241" style="border: 0; float: left;" alt="Нажмите отобразить рисунки">
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/bot_line_white.jpg" width="800" height="8" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                </table>
                
                <!-- Разделитель -->
                <table width="800" height="20" cellspacing="0" cellpadding="0" align="center" bgcolor="#808080"  style="height: 20px; border:0; border-collapse: collapse; overflow: hidden;">
                    <tr style="line-height: 0; height: 20px;"><td style="line-height: 0; height: 20px;"><span style="overflow: hidden; margin: 0; padding: 0; color: #808080; line-height: 0; height: 20px;">_</span></td></tr>
                </table>
                
                <!-- Вторая строка с двумая блоками-->
                <table width="800" cellspacing="0" cellpadding="0" align="center" bgcolor="#808080"  style="border:0; border-collapse: collapse;">
                    <tr>
                        <td style="width: 390px; padding-right: 10px;" valign="top" align="left">
                            <table width="390" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/top_line_white.jpg" width="390" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                                </tr>
                                <tr>
                                    <td style="padding: 10px 20px;">
                                        <table width="350" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                            <tr>
                                                <td valign="center" style="padding-right: 15px; width: 16px;">
                                                    <img src="http://www.lbr.ru/images/kp/time_to_by_kp/unia-logo.jpg" width="16" height="25" style="border: 0; float: left;" alt="Нажмите отобразить рисунки">
                                                </td>
                                                <td valign="center">
                                                    <span style="font-size: 13pt; font-weight: bold; font-family: Calibri; color: black; text-transform: uppercase;">Культиватор предпосевной Viking</span>
                                                </td>
                                            </tr>
                                        </table>
                                        <table width="350" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                            <tr>
                                                <td>
                                                   <img src="http://www.lbr.ru/images/kp/time_to_by_kp/march/2.jpg" width="350" height="129" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"> 
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 10px 10px 10px 0;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; font-weight: bold;">Полная качественная подготовка поля к посеву за один  проход:</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 0px 10px 10px 0;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: justify; line-height: 13pt;">культивация | рыхление | выравнивание | прикатка</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 0px 10px 10px 0;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left;"><b>Результат:</b>  полностью выровненная поверхность с уплотненным ложем для семян в посевном слое почвы; отсутствие гребней, глыб, следов.</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="text-align: right;">
                                                    <a href="http://www.lbr.ru/tehnika/klassicheskie-tekhnologii-pochvoobrabotki-i-seva/kultivatory-predposevnye/337-kultivatory-viking/?utm_source=http%3A%2F%2Fwww.lbr.ru%2Fgetkp%2Ftest%2Fv%2Fv57%2F&utm_medium=email&utm_term=VIKING&utm_campaign=VIKING" title="Культиваторы предпосевные Viking" style="border: 0; text-decoration: none;">
                                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/more.jpg" width="113" height="25" style="border:0; float: right;">
                                                    </a>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
                                <tr>
                                    <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/bot_line_white.jpg" width="390" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                                </tr>
                            </table>
                        </td>
                        <td style="width: 390px; padding-left: 10px;" valign="top" align="right">
                            <table width="390" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/top_line_white.jpg" width="390" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                                </tr>
                                <tr>
                                    <td style="padding: 10px 20px;">
                                        <table width="350" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                            <tr>
                                                <td valign="center" style="padding-right: 15px; width: 16px;">
                                                    <img src="http://www.lbr.ru/images/kp/time_to_by_kp/unia-logo.jpg" width="16" height="25" style="border: 0; float: left;" alt="Нажмите отобразить рисунки">
                                                </td>
                                                <td valign="center">
                                                    <span style="font-size: 13pt; font-weight: bold; font-family: Calibri; color: black; text-transform: uppercase;">Пружинная борона UNIA Akzent</span>
                                                </td>
                                            </tr>
                                        </table>
                                        <table width="350" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                            <tr>
                                                <td>
                                                   <img src="http://www.lbr.ru/images/kp/time_to_by_kp/march/3.jpg" width="350" height="146" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"> 
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 20px 10px 20px 0;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: justify; line-height: 13pt;">Разрушить почвенную  корку при выращивании зерновых культур? Уничтожить сорняки без гербицидов на полях с засеянным картофелем, кукурузой, свеклой или другими овощными культурами? Это  задачи для  пружинной бороны Akzent!</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="text-align: right;">
                                                    <a href="http://www.lbr.ru/tehnika/klassicheskie-tekhnologii-pochvoobrabotki-i-seva/borony-pruzhinnye/334-pruzhinnye-borony-akzent/" title="Бороны пружинные Akzent" style="border: 0; text-decoration: none;">
                                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/more.jpg" width="113" height="25" style="border:0;  float: right;">
                                                    </a>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
                                <tr>
                                    <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/bot_line_white.jpg" width="390" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                </table>
                
                <!-- Разделитель -->
                <table width="800" height="20" cellspacing="0" cellpadding="0" align="center" bgcolor="#808080"  style="height: 20px; border:0; border-collapse: collapse; overflow: hidden;">
                    <tr style="line-height: 0; height: 20px;"><td style="line-height: 0; height: 20px;"><span style="overflow: hidden; margin: 0; padding: 0; color: #808080; line-height: 0; height: 20px;">_</span></td></tr>
                </table>
                
                <!-- Третья строка с одним блоком -->
                <table width="800" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/top_line_white.jpg" width="800" height="8" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 20px 5px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="top" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td valign="top" style="width: 370px; padding-right: 10px;">
                                        <table width="370" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                            <tr>
                                                <td valign="center" style="padding-right: 15px; width: 16px;">
                                                    <img src="http://www.lbr.ru/images/kp/time_to_by_kp/unia-logo.jpg" width="16" height="25" style="border: 0; float: left;" alt="Нажмите отобразить рисунки">
                                                </td>
                                                <td valign="center">
                                                    <span style="font-size: 13pt; font-weight: bold; font-family: Calibri; color: black; text-transform: uppercase;">Плуг оборотный навесной UNIA Ibis</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td colspan="2">
                                                    <img src="http://www.lbr.ru/images/kp/time_to_by_kp/march/4.jpg" width="370" height="185" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"> 
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                    <td valign="top" style="width: 380px;">
                                        <table width="380" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                            <tr>
                                                <td style="padding: 25px 0;">
                                                    <p style="padding: 0; font-size: 12pt; font-family: Calibri; color: black; margin: 0; text-align: justify; line-height: 13pt; font-weight: bold;">1500 шт. на полях России – результат экономичности, простоты и надежности!</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 0px 0 20px;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: justify; line-height: 14pt;">Плуги Ibis предназначены для безотвальной вспашки на любых типах почвы. В результате работы плуга происходит выравнивание обрабатываемых полей, что позволяет в дальнейшем применять менее </p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="text-align: right;">
                                                    <a href="http://www.lbr.ru/tehnika/klassicheskie-tekhnologii-pochvoobrabotki-i-seva/plugi/323-plugi-oborotnye-navesnye-ibis/" title="Плуги оборотные навесные Ibis" style="border: 0; text-decoration: none;">
                                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/more.jpg" width="113" height="25" style="border:0; float: right;">
                                                    </a>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/bot_line_white.jpg" width="800" height="8" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                </table>
                
                <!-- Разделитель -->
                <table width="800" height="20" cellspacing="0" cellpadding="0" align="center" bgcolor="#808080"  style="height: 20px; border:0; border-collapse: collapse; overflow: hidden;">
                    <tr style="line-height: 0; height: 20px;"><td style="line-height: 0; height: 20px;"><span style="overflow: hidden; margin: 0; padding: 0; color: #808080; line-height: 0; height: 20px;">_</span></td></tr>
                </table>
                
                <!-- Четвертая строка с одним блоком -->
                <table width="800" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/top_line_white.jpg" width="800" height="8" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 20px 0px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td valign="center" style="padding-right: 15px; width: 22px;">
                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/expom-logo.jpg" width="22" height="25" style="border: 0; float: left;" alt="Нажмите отобразить рисунки">
                                    </td>
                                    <td valign="center">
                                        <span style="font-size: 13pt; font-weight: bold; font-family: Calibri; color: black; text-transform: uppercase;">Прикатывающие катки EXPOM Jacek и Maximus</span>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 10px 20px 5px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="top" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td valign="top" style="width: 370px; padding-right: 10px;">
                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/march/5.jpg" width="370" height="146" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"> 
                                    </td>
                                    <td valign="top" style="width: 380px;">
                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/march/6.jpg" width="380" height="146" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"> 
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 20px 5px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="top" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td style="padding-bottom: 10px;">
                                        <p style="padding: 0; font-size: 13pt; font-family: Calibri; color: black; margin: 0; text-align: justify; line-height: 13pt; font-weight: bold;">Рост урожайности на 10-15%</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding-bottom: 5px;">
                                        <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 13pt;">Прикатывающих катков JACEK И MAXIMUS  - это готовое решение для прикатки после простых сеялок с загортачами для ускорения всходов.</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding-bottom: 5px;">
                                        <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 13pt;">Уплотняет пахотный слой - создаёт лучший контакт семян с почвой. </p>
                                        <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 13pt;">Культивирует. Выравнивает поверхность. Дробит глыбы</p>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="text-align: right;">
                                        <a href="http://www.lbr.ru/tehnika/klassicheskie-tekhnologii-pochvoobrabotki-i-seva/prikatyvayushchie-katki/657-prikatyvayushchie-katki-expom/" title="Прикатывающие катки Expom" style="border: 0; text-decoration: none;">
                                            <img src="http://www.lbr.ru/images/kp/time_to_by_kp/more.jpg" width="113" height="25" style="border:0; float: right;">
                                        </a>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/bot_line_white.jpg" width="800" height="8" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                </table>
                
                <!-- Разделитель -->
                <table width="800" height="20" cellspacing="0" cellpadding="0" align="center" bgcolor="#808080"  style="height: 20px; border:0; border-collapse: collapse; overflow: hidden;">
                    <tr style="line-height: 0; height: 20px;"><td style="line-height: 0; height: 20px;"><span style="overflow: hidden; margin: 0; padding: 0; color: #808080; line-height: 0; height: 20px;">_</span></td></tr>
                </table>
                
                <!-- Пятая строка с одним блоком -->
                <table width="800" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/top_line_white.jpg" width="800" height="8" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 20px 0px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td valign="center" style="padding-right: 15px; width: 16px;">
                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/unia-logo.jpg" width="16" height="25" style="border: 0; float: left;" alt="Нажмите отобразить рисунки">
                                    </td>
                                    <td valign="center">
                                        <span style="font-size: 13pt; font-weight: bold; font-family: Calibri; color: black; text-transform: uppercase;">Разбрасыватель удобрений UNIA MX 950H</span>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 10px 20px 5px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td style="width: 370px; padding-right: 10px;">
                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/march/7.jpg" width="370" height="189" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"> 
                                    </td>
                                    <td valign="top">
                                        <table width="380" cellspacing="0" cellpadding="0" align="top" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                            <tr>
                                                <td style="padding-bottom: 10px;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 13pt;">"Разбрасыватель MX компании UNIA обеспечивает правильное и точное внесение удобрений" – сделала заключение по результатам испытаний  независимая ассоциация  сельхозпроизводителей Германии DLG.  </p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 0 0 20px;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: justify; line-height: 13pt;">А мы можем добавить:</p>
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: justify; line-height: 13pt; font-weight: bold;">… и 30% выигрыш в цене по сравнению с европейскими аналогами!</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="text-align: right;">
                                                    <a href="http://www.lbr.ru/tehnika/vnesenie-udobrenij/navesnye-razbrasyvateli-mineralnykh-udobrenij/431-razbrasyvateli-mineralnykh-udobrenij-navesnye-mx/" title="Навесные разбрасыватели минеральных удобрений МХ" style="border: 0; text-decoration: none;">
                                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/more.jpg" width="113" height="25" style="border:0; float: right;">
                                                    </a>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/bot_line_white.jpg" width="800" height="8" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                </table>
                
                <!-- Разделитель -->
                <table width="800" height="20" cellspacing="0" cellpadding="0" align="center" bgcolor="#808080"  style="height: 20px; border:0; border-collapse: collapse; overflow: hidden;">
                    <tr style="line-height: 0; height: 20px;"><td style="line-height: 0; height: 20px;"><span style="overflow: hidden; margin: 0; padding: 0; color: #808080; line-height: 0; height: 20px;">_</span></td></tr>
                </table>
                
                <!-- Шестая строка с одним блоком -->
                <table width="800" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/top_line_white.jpg" width="800" height="8" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                    <tr>
                        <td style="padding: 5px 20px 0px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td valign="center" style="padding-right: 15px; width: 54px;">
                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/dieci-logo.jpg" width="54" height="14" style="border: 0; float: left;" alt="Нажмите отобразить рисунки">
                                    </td>
                                    <td valign="center">
                                        <span style="font-size: 13pt; font-weight: bold; font-family: Calibri; color: black; text-transform: uppercase;">ТЕЛЕСКОПИЧЕСКИЙ ПОГРУЗЧИК Dieci Agri Farmer 30.7</span>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 10px 20px 5px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td style="width: 370px; padding-right: 10px;">
                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/march/8.jpg" width="370" height="276" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"> 
                                    </td>
                                    <td valign="top">
                                        <table width="380" cellspacing="0" cellpadding="0" align="top" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                            <tr>
                                                <td style="padding: 20px 0 0;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 15pt;">Быстрая и легкоуправляемая  машина с многочисленными рабочими органами, такими как,  например, ковши, захваты для рулонов, разнообразные вилы и т.д. делают её обязательной в парке техники для всех хозяйств.</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 20px 0 30px;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 13pt; font-weight: bold;">КРАТКО о ПОГРУЗЧИКЕ можно сказать следующее:  </p>
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 13pt; font-weight: bold; padding-left: 15px;">НЕДОРОГОЙ.</p>
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 13pt; font-weight: bold; padding-left: 15px;">МНОГОФУНЦИОНАЛЬНЫЙ.</p>
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 13pt; font-weight: bold; padding-left: 15px;">ЭКОНОМИЧНЫЙ.</p>
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 13pt; font-weight: bold; padding-left: 15px;">ТОЧНЫЙ.</p>
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: left; line-height: 13pt; font-weight: bold; padding-left: 15px;">ПРОИЗВОДИТЕЛЬНЫЙ.</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="text-align: right;">
                                                    <a href="http://www.lbr.ru/tehnika/teleskopicheskie-pogruzchiki/dieci/300-teleskopicheskie-pogruzchiki-serii-agri-farmer/" title="Телескопические погрузчики серии Agri Farmer" style="border: 0; text-decoration: none;">
                                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/more.jpg" width="113" height="25" style="border:0; float: right;">
                                                    </a>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/bot_line_white.jpg" width="800" height="8" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                </table>
                
                <!-- Разделитель -->
                <table width="800" height="20" cellspacing="0" cellpadding="0" align="center" bgcolor="#808080"  style="height: 20px; border:0; border-collapse: collapse; overflow: hidden;">
                    <tr style="line-height: 0; height: 20px;"><td style="line-height: 0; height: 20px;"><span style="overflow: hidden; margin: 0; padding: 0; color: #808080; line-height: 0; height: 20px;">_</span></td></tr>
                </table>
                
                <!-- Седьмая строка с одним блоком  DIECI с полосками-->
                <table width="800" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/top_line_dieci.jpg" width="800" height="19" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                    <tr>
                        <td style="padding: 15px 20px 0px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td valign="center">
                                        <span style="font-size: 13pt; font-weight: bold; font-family: Calibri; color: #E31E24; margin: 0; padding-right: 10px;">ТЕСТ-ДРАЙВ</span>
                                        <span style="font-size: 13pt; font-weight: bold; font-family: Calibri; color: black; text-transform: uppercase;">Приглашение на Тест-драйв DIECI и Акцию по DIECI</span>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 10px 20px 15px;">
                            <table width="760" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                <tr>
                                    <td style="width: 370px; padding-right: 10px;">
                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/march/9.jpg" width="370" height="278" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"> 
                                    </td>
                                    <td valign="top">
                                        <table width="380" cellspacing="0" cellpadding="0" align="top" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                                            <tr>
                                                <td style="padding: 20px 0 0;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: justify; line-height: 13pt;">Планируете приобрести для хозяйства погрузчик, но не знаете на каком остановить свой выбор?</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 20px 0;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: justify; line-height: 13pt;">Приглашем к нам на торговую площадку ЛБР-Агромаркет!! Где Вы сможете проверить работу флагмана в мире погрузчиков DIECI в действии!!</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 0 0 50px;">
                                                    <p style="padding: 0; font-size: 10pt; font-family: Calibri; color: black; margin: 0; text-align: justify; line-height: 13pt;" style="text-align: left;">И рекомендуем воспользоваться специальным предложением – дополнительной <b>скидкой 7%</b>  - при покупки рабочих органов с погрузчиком!</p>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="text-align: right;">
                                                    <a href="http://www.lbr.ru/selskohozyaystvennaya-tehnika/type/teleskopicheskie-pogruzchiki/" title="ВЫБЕРИ СВОЙ ПОГРУЗЧИК" style="border: 0; text-decoration: none;">
                                                        <img src="http://www.lbr.ru/images/kp/time_to_by_kp/more.jpg" width="113" height="25" style="border:0; float: right;">
                                                    </a>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <tr>
                        <td><img src="http://www.lbr.ru/images/kp/time_to_by_kp/bot_line_dieci.jpg" width="800" height="19" style="border: 0; float: left;" alt="Нажмите отобразить рисунки"></td>
                    </tr>
                </table>
                
                <!-- END -->
            </td>
        </tr>
    </table>
    <!-- Контакты -->
    <table width="840" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="height: 10px; border:0; border-collapse: collapse; overflow: hidden; ">
        <tr>
            <td colspan="2" style="padding: 15px 0 0;">
                <p style="padding: 0; font-size: 12pt; font-weight: bold; font-family: Calibri; color: #333333; margin: 0; text-align: justify; line-height: 13pt;">Ждем Вас в филиале ЛБР-АгроМаркет (<?php echo $filial['name']; ?>)!</p>
                <p style="padding: 0; font-size: 11pt; font-family: Calibri; color: #333333; margin: 0; text-align: justify; line-height: 13pt;">Время работы: <?php echo $filial['work_time']; ?></p>
                <p style="padding: 0; font-size: 11pt; font-family: Calibri; color: #333333; margin: 0; text-align: justify; line-height: 13pt;">Адрес филиала: <?php echo $filial['address']; ?></p>
                <p style="padding: 0; font-size: 11pt; font-family: Calibri; color: #333333; margin: 0; text-align: justify; line-height: 13pt;">Телефон: <?php echo $filial['telephone']; ?></p>
            </td>
        </tr>
        <tr>
            <td style="padding: 20px 10px 10px 0; width: 390px;">
                <table width="390" cellspacing="0" cellpadding="0" align="left" bgcolor="#FFFFFF"  style="height: 10px; border:0; border-collapse: collapse; overflow: hidden; ">
                    <tr>
                        <td width="120">
                            <img src="http://www.lbr.ru<?php echo $filial['director_photo']; ?>" width="98" height="146" style="border: 0; float: left;" alt="Нажмите отобразить рисунки">
                        </td>
                        <td valign="top">
                            <p style="padding: 0; font-size: 12pt; font-weight: bold; font-family: Calibri; color: #333333; margin: 0; text-align: left; line-height: 13pt;">Директор филиала:</p>
                            <p style="padding: 0; font-size: 11pt; font-family: Calibri; color: #333333; margin: 0; text-align: left; line-height: 13pt;"><?php echo $filial['director_surname'].' '.$filial['director_name'].' '.$filial['director_secondname']; ?></p>
                            <p style="padding: 0; font-size: 11pt; font-family: Calibri; color: #333333; margin: 0; text-align: left; line-height: 13pt;">Телефон: <?php echo $filial['director_phone']; ?></p>
                            <p style="padding: 0; font-size: 11pt; font-family: Calibri; color: #333333; margin: 0; text-align: left; line-height: 13pt;">E-mail: <?php echo $filial['director_email']; ?></p>
                        </td>
                    </tr>
                </table>
            </td>
            <td style="padding: 20px 0 10px; width: 400px;" valign="top">
                <table width="400" cellspacing="0" cellpadding="0" align="left" bgcolor="#FFFFFF"  style="height: 10px; border:0; border-collapse: collapse; overflow: hidden; ">
                    <tr>
                        <td valign="top">
                            <p style="padding: 0; font-size: 12pt; font-weight: bold; font-family: Calibri; color: #333333; margin: 0; text-align: justify; line-height: 13pt;">Ваш персональный менеджер:</p>
                            <p style="padding: 0; font-size: 11pt; font-family: Calibri; color: #333333; margin: 0; text-align: left; line-height: 13pt;"><?php echo $data['user_info']; ?></p>
                        </td>
                    </tr>
                </table>
            </td>
        </tr>
    </table>

    <!-- Разделитель -->
    <table width="840" height="20" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="height: 20px; border:0; border-collapse: collapse; overflow: hidden;">
        <tr style="line-height: 0; height: 20px;"><td style="line-height: 0; height: 20px;"><span style="overflow: hidden; margin: 0; padding: 0; color: #FFFFFF; line-height: 0; height: 20px;">_</span></td></tr>
    </table>


    <table width="840" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
        <tr>
            <td style="width: 170px; padding: 10px 0; ">
                <a href="http://www.lbr.ru/" title="Сайт ЛБР-агромаркет" style="color: #333333; text-decoration: none; font-size: 10pt; font-family: 'Trebuchet MS'; padding: 0;" style="font-weight: bold; font-size: 12pt;">www.lbr.ru</a>
            </td>
            <td style="text-align: center; padding-bottom: 10px; padding-right: 10px" valign="bottom">
                <!-- Menu -->
                <table width="350" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse;">
                    <tr>
                        <!-- Первый пункт меню -->
                        <td>
                            <table width="120" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse; border-right: 1px solid #333333; text-align: center;">
                                <tr>
                                    <td><a href="http://www.lbr.ru/selskohozyaystvennaya-tehnika/type/" title="Сельскохозяйственная техника" style="color: #333333; text-decoration: none; font-size: 10pt; font-family: 'Trebuchet MS'; padding: 0;">Сельхозтехника</a></td>
                                </tr>
                            </table>
                        </td>
                        <!-- Второй пункт меню -->
                        <td>
                           <table width="80" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse; border-right: 1px solid #333333; text-align: center;">
                               <tr>
                                   <td><a href="http://www.lbr.nichost.ru/spareparts/?c=83" title="Запасный части" style="color: #333333; text-decoration: none; font-size: 10pt; font-family: 'Trebuchet MS'; padding: 0;">Запчасти</a></td>
                               </tr>
                           </table>
                       </td>
                       <!-- Третий пункт меню -->
                        <td>
                            <table width="70" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse; border-right: 1px solid #333333; text-align: center;">
                                <tr>
                                    <td><a href="http://www.lbr.ru/service/" title="Сервис" style="color: #333333; text-decoration: none; font-size: 10pt; font-family: 'Trebuchet MS'; padding: 0;">Сервис</a></td>
                                </tr>
                            </table>
                        </td>
                        <!-- Четверты пункт меню -->
                        <td>
                            <table width="80" cellspacing="0" cellpadding="0" align="center" bgcolor="#FFFFFF"  style="border:0; border-collapse: collapse; text-align: center;">
                                <tr>
                                    <td><a href="http://www.lbr.ru/company/contacts/" title="Контакты ЛБР-Агромаркет" style="color: #333333; text-decoration: none; font-size: 10pt; font-family: 'Trebuchet MS'; padding: 0;">Контакты</a></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                </table>
            </td>
        </tr>
    </table>
</body>