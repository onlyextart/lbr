...
class MapItems extends CActiveRecord
{
	...
	public function search()
	{
        ...
        static function getMenuTree2($products) {
            $categories = Yii::app()->db->createCommand('SELECT id,level,name,published FROM menu_items WHERE lft NOT BETWEEN 642 AND 1249 ORDER BY lft')->queryAll();
            
            return(self::toHierarchy2($categories, 'getMenuManageRow', $products));
        }
        ...
        static function getProductList() 
        {
            ...
            $resultArray = array();
            foreach($categoriesLeaf as $k => $value) {
                $str = substr($categoriesLeaf[$k]['path'], 0, strpos($categoriesLeaf[$k]['path'], '/' . $categoriesLeaf[$k]['alias']));
        ...
        private static function toHierarchy2($collection, $rowBuildFunction, $products, $checkBoxNameAttr = null, $checkStatus = null, $activeItemType=null)
        {
            // Trees mapped
            $trees = array();
            $l = 0;
            ...
            if (count($collection) > 0) {
                ...
                foreach ($collection as $node) {
                    ...
                    eval('$item[text] = self::' . $rowBuildFunction . '($item, $checkBoxNameAttr, $checkStatus, $activeItemType);');
                    $item['children'] = array();
                    /*Yii::log($item[id],'info');
                    if(array_key_exists($item['id'], $products)){
                        //foreach($products[$item['id']] as $value){
                            $item['children'][] = array(
                                'id' => 'addMenuItem',
                                'text' => 'Node 1',
                                'expanded' => false,
