...
class MapItems extends CActiveRecord
{
	...
	public function search()
	{
        ...
        static function getMenuTree() {
            $categories = Yii::app()->db->createCommand('SELECT id,level,name,published FROM menu_items WHERE lft NOT BETWEEN 642 AND 1249 ORDER BY lft')->queryAll();
            // leafs
            $categoriesLeaf = Yii::app()->db->createCommand('SELECT id, level, name, published, path, alias FROM menu_items WHERE type = 1')->queryAll();
            //var_dump($categories); exit();
            //$categories = array_merge($categories2,$categoriesLeaf);        
            $resultArray = array(); // хранит url продукта и его родителя
            $count = sizeof($categoriesLeaf);
            foreach($categoriesLeaf as $value) {
                $array = array();
                foreach($value as $key => $value2) {
                    if($key == 'path') {
                        $array[$key] = $value2;
                    } else if($key == 'alias'){
                        $str = substr($array['path'], 0, strpos($array['path'], '/' . $value2));
                        $array['parent'] = $str; 
                        $id = Yii::app()->db->createCommand()
                            ->select('id')
                            ->from('menu_items')
                            ->where(array('like', 'path', $str))
                            ->queryRow()
                        ;
                        $array['parentId'] = $id['id'];
            ...
            echo '=========================================<br/>'; 
            */
            //$categoriesLeaf = Yii::app()->db->createCommand('SELECT id,level,name,published FROM menu_items WHERE type = 1 and ')->queryAll();
            
            //exit();
            
            return(self::toHierarchy($categories, 'getMenuManageRow', $resultArray));
        }
        
        private static function toHierarchy($collection, $rowBuildFunction, $resultArray, $checkBoxNameAttr = null, $checkStatus = null, $activeItemType=null)
        {
            // Trees mapped
            $trees = array();
            $l = 0;
            ...
            /*foreach($resultArray as $key => $value) { 
                  foreach($value as $key2 => $value2) {
                      echo $key2 . ' = ' . $value2 . '<br/>';
                      if($key2 == 'parentId') {
                         self::addProductsToTree($value2, $trees); 
                         echo '<br>';
                      }
                      //echo $key2 . ' = ' . $value2 . '<br/>';
                  }
            ...
            //exit;
            //return(self::addProductsToTree(94, $trees));
            return $trees;
        }
        
        private static function addProductsToTree($var, $text)
        {
            /*$result = false;
            foreach($array as $id => $childarray) {
                $filteredparents = self::addProductsToTree($matchid, $childarray["children"]);
                if ($childarray['id'] == $matchid || $filteredparents) {
                    //$childarray["children"] = $filteredparents;
                    //echo '1111 <br>';
                    //echo $childarray['id'], '<br>';
            ...
            foreach ($var as $val)
            {
                if(is_array($val) && self::addProductsToTree($var, $text)) return true;
                elseif ($val == $text) {
                    echo 'd <br>';
                    return true;
                }
